...
router.post("/", validateTask, async (req: Request, res: Response) => {
  try {
    // Buscar el id máximo existente en MongoDB
    // y asignar un nuevo id único
    // Se busca el id máximo en la base de datos MongoDB
    // y se le suma 1 para crear un nuevo id único
    const maxIdDoc = await tareasCollection
      .find()
      .sort({ id: -1 })
      .limit(1)
      .toArray();
    ...
    if (maxIdDoc.length > 0) {
      newId = maxIdDoc[0].id + 1;
    }

    // Crear la tarea con id único
    const task: Task = {
      id: newId,
    ...
    await tareasCollection.insertOne(task);
...
router.put("/:id", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al marcar la tarea como completada." }); // aqui hay un error en el local
  }
});
